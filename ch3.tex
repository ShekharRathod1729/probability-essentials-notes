...
\begin{problem}
  Suppose we model tossing a coin with two outcomes, $H$ and $T$, representing Heads and Tails. Let $P(H) = P(T) = \frac{1}{2}$. Suppose now we toss two such coins, so that the sample space of outcomes $\Omega$ consists of four points: $HH$, $HT$, $TH$, $TT$. We assume that the tosses are independent. 
  (a) Find the conditional probability that both coins show a head given that the first shows a head.
  (b) Find the conditional probability that both coins show heads given that at least one of them is a head.
\end{problem}
